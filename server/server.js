...
app.get("/match/:match_id/bowling/:innings", async (req, res) => {
    try {
        const { match_id, innings } = req.params;
        const allTodos = await pool.query(`SELECT * from (SELECT player.player_id, player.player_name as bowler,
        ...
        SUM(CASE  WHEN ball_by_ball.out_type='caught' OR ball_by_ball.out_type='caught and bowled' 
        OR ball_by_ball.out_type='bowled' OR ball_by_ball.out_type='stumped' 
        
        OR ball_by_ball.out_type='keeper catch' OR ball_by_ball.out_type='lbw' OR ball_by_ball.out_type='hit wicket' THEN 1 ELSE 0 END)
         AS wickets
        FROM player, ball_by_ball
        ...
        where i.wickets > 0 order by i.wickets DESC, runs_given ASC, bowler ASC`
            , [match_id, innings]);
        res.json(allTodos.rows);
    } catch (err) {
        console.error(err.message);
